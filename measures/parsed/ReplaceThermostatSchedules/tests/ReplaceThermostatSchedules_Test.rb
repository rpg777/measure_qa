...
class ReplaceThermostatSchedules_Test < MiniTest::Test
  ...
  def test_ReplaceThermostatSchedules
     
    # create an instance of the measure
    measure = ReplaceThermostatSchedules.new
    ...
    zone_to_heating_sch_map = Hash.new
    zone_to_cooling_sch_map = Hash.new
    model.getThermalZones.each do |zone|
      ...
      if thermostatSetpointDualSetpoint.is_initialized
        ...
        heating = thermostatSetpointDualSetpoint.heatingSetpointTemperatureSchedule
        if heating.is_initialized
          zone_to_heating_sch_map[zone.name.get] = heating.get.name.get
        end
        cooling = thermostatSetpointDualSetpoint.coolingSetpointTemperatureSchedule
        if cooling.is_initialized
          zone_to_cooling_sch_map[zone.name.get] = cooling.get.name.get
        end
      end
    end
    assert_equal(1, zone_to_heating_sch_map.size)
    assert_equal(1, zone_to_cooling_sch_map.size)
    ...
    model.getThermalZones.each do |zone|
      ...
      if thermostatSetpointDualSetpoint.is_initialized
        ...
        assert(heating.get.name.get == zone_to_heating_sch_map[zone.name.get])
        cooling = thermostatSetpointDualSetpoint.get.coolingSetpointTemperatureSchedule
        assert(cooling.is_initialized)
        assert(cooling.get.name.get == zone_to_cooling_sch_map[zone.name.get])
        num_zones += 1
      end
    end
