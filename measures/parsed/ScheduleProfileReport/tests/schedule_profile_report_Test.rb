...
class ScheduleProfileReport_Test < MiniTest::Test
    
  # paths to expected test files, includes osm and eplusout.sql
  def modelPath
    return "#{File.dirname(__FILE__)}/ExampleModel.osm" #while this seems un-necessary, the reporting measure will fail if it can't find the model.
  end
  ...
  def idfPath
    return "#{File.dirname(__FILE__)}/ExampleModel.idf"
  end
  ...
  def resultsPath
    return "#{File.dirname(__FILE__)}/EnergyPlusPreProcess/"
  end

  def sqlPath
    return "#{File.dirname(__FILE__)}/EnergyPlusPreProcess/EnergyPlus-0/eplusout.sql" # old path ExampleModel/ModelToIdf/EnergyPlusPreProcess-0/EnergyPlus-0/eplusout.sql
  end
  ...
  # the actual test
  def test_ScheduleProfileReport
     
    assert(File.exist?(modelPath()))
    assert(File.exist?(sqlPath()))
     
    # create an instance of the measure
    measure = ScheduleProfileReport.new
